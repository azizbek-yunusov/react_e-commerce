  ...
  return (
    <div className="md:hidden block w-full rounded-t-[30px]  bg-gray-200 fixed left-0 bottom-0 shadow-xl">
      <div className="container-full flex items-center justify-between px-8 py-3">
        <Link to={"/"} className='p-2 rounded-full text-zinc-800 bottom-1 active:text-yellow-500 transition-all duration-100 ease-linear align-middle border-gray-500'>{HomeIcon}</Link>
        <div onClick={() => dispatch(handleSideBar())} className='p-2 rounded-full align-middle text-zinc-800 active:text-yellow-500 transition-all duration-100 ease-linear'>
        ...
        <Cart />
        <div onClick={() => dispatch(handleSignIn())} className='p-2 rounded-full text-zinc-800 active:text-yellow-500 transition-all duration-100 ease-linear'>{UserIcon}</div>
      </div>
    </div>
  )
}
