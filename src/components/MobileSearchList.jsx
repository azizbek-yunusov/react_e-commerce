    ...
    <div className="bg-white animatedMobile lg:hidden fixed inset-0 z-50 min-w-full min-h-screen">
      <div className="w-full">
        <div className="flex px-3 justify-between w-full items-center py-4 bg-gray-100">
          ...
          <button
            onClick={CloseBar}
            className="p-2 bottom-0 rounded-full border active:border active:border-gray-200 transition text-zinc-600"
          >
            {CloseIcon}
          </button>
        </div>
            ...
            {filteredData.slice(0, 20).map((value, key) => {
              return (
                <Link
                  ...
                  onClick={CloseBar}
                  className="cursor-pointer py-4 w-full px-5 active:bg-yellow-50 flex text-left"
                  key={key}
                >
                  <img
                    src={value.imgUrl[0]}
