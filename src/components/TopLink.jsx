import React from "react";
import { Link } from "react-router-dom";
import { LocationIcon, PhoneIcon } from "./svg";

const TopLink = () => {
  return (
    <div className="bg-zinc-700 -z-10 w-full border-b border-b-gray-900">
      <div className="container-full lg:py-3 py-[6px] flex items-center justify-between">
        <div className="flex">
          <Link
            className="lg:pr-11 pr-2 text-red-600 font-bold lg:text-4xl text-xl lg:hidden block"
            to="/"
          >
            Logo.uz
          </Link>

          <div className="lg:flex hidden">
            <img
              className="align-middle"
              width={30}
              height={30}
              src="data:image/svg+xml;base64,"
              alt=""
            />
            <p className="text-white ml-2">Рус</p>
          </div>
          <div className="text-white lg:ml-8 ml-4 lg:flex hidden">
            {LocationIcon} <p className="ml-1">Узбекистан</p>
          </div>
        </div>
        <div className="flex">
          <div className="flex">
            <div className="lg:hidden cursor-pointer text-white">
              {PhoneIcon}
            </div>
            <p className="text-white lg:block hidden text-lg font-medium">
              Контакт центр:
            </p>
            <p className="text-white lg:block hidden text-lg font-medium ml-3">
              +998(94) 554 55 94
            </p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default TopLink;
